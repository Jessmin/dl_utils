...
def main():
    ...
    for i in progressbar.progressbar(range(len(label_file_list))):
        ...
        if imageData:
            ...
            _,filename = os.path.split(data['imagePath'])
            img_file = osp.join(osp.dirname(label_file), f'../images/{filename}')
        ...
        for shape in data['shapes']:
            ...
            cnt = np.asarray(shape['points'], np.float32)
        ...
        for shape in sorted(data['shapes'], key=lambda x: x['label']):
            ...
            if label_name in label_name_to_value:s
